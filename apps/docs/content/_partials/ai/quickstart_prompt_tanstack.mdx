<PromptPanel>
  <Prompt value="prompt" expandable>
    <PromptTitle icon={<Sparkles />}>AI Prompt</PromptTitle>
    <PromptCopy>{`Help me add Supabase to my TanStack Start project. Create a Supabase project at
database.new and run the instruments table SQL. Then:
1. Run \`npm create @tanstack/start@latest my-app\` to scaffold the app.
2. Run \`npm install @supabase/supabase-js\`.
3. Create \`.env\` and set \`VITE_SUPABASE_URL\` and
   \`VITE_SUPABASE_PUBLISHABLE_KEY\`.
4. Create \`src/utils/supabase.ts\` to initialize the Supabase client.
5. Update \`src/routes/index.tsx\` with a loader that queries and displays the
   instruments table.
6. Run \`npm run dev\` and open http://localhost:3000.`}</PromptCopy>
    <PromptContent>{`Help me add Supabase to my TanStack Start project. Create a Supabase project at
database.new and run the instruments table SQL. Then:
1. Run \`npm create @tanstack/start@latest my-app\` to scaffold the app.
2. Run \`npm install @supabase/supabase-js\`.
3. Create \`.env\` and set \`VITE_SUPABASE_URL\` and
   \`VITE_SUPABASE_PUBLISHABLE_KEY\`.
4. Create \`src/utils/supabase.ts\` to initialize the Supabase client.
5. Update \`src/routes/index.tsx\` with a loader that queries and displays the
   instruments table.
6. Run \`npm run dev\` and open http://localhost:3000.`}</PromptContent>
  </Prompt>
</PromptPanel>
