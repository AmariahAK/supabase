...
Use Tailwind `ring`, not `outline`. Import the shared constants from `ui` instead of inventing local styles:

```tsx
import { focusRing, focusRingInset } from 'ui'
```

**Default** (`focusRing`) — buttons, inputs, and most controls:
...
**Inset** (`focusRingInset`) — dense or flush surfaces such as interactive table rows (Studio also exposes this as the `.inset-focus` utility class):
...
- Do not animate the focus ring; keyboard navigation is high-frequency UI
