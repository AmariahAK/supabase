...
dark-theme-only Supabase treatment, or the reverse. When either side is a
static or light asset (most partner icons), keep `SupabaseLogo` on its light
mark too. Theme-aware dark variants are fine for curated partners that already
have them, but then both sides of the pair should use the dark set together.

**Uploaded icons force light tiles.** Organisation OAuth app icons from Studio
are unclassified bitmaps. When pairing one with `SupabaseLogo`, use fixed light
tile chrome (`bg-white` / `SupabaseLogo forceLight`) on both sides in light and
dark Studio themes. Do not invent a dark variant for the upload.
...
```tsx
// Trusted / curated requester: AuthorizeConnectLogo pairs with Supabase
<AuthorizeConnectLogo icon={icon} name={name} />

// Unknown or unusable requester icon: AuthorizeConnectLogo renders Supabase alone
<AuthorizeConnectLogo icon={null} name="Acme" />
