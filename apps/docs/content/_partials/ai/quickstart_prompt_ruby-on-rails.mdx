<PromptPanel>
  <Prompt value="prompt" expandable>
    ...
    <PromptCopy>{`Help me add Supabase to my Ruby on Rails project. Create a Supabase project at
...
5. Run \`bin/rails server\` and open http://127.0.0.1:3000.`}</PromptCopy>
    <PromptContent>{`Help me add Supabase to my Ruby on Rails project. Create a Supabase project at
database.new and run the instruments table SQL. Then:
1. Run \`rails new blog -d=postgresql\` to scaffold a new Rails project.
2. Set \`DATABASE_URL\` to the Supabase Session Pooler connection string in
   \`.env\`.
3. Generate an Instrument model with \`bin/rails generate model Instrument
   name:string --skip-migration\`.
4. Use \`bin/rails console\` to create and query instruments.
5. Run \`bin/rails server\` and open http://127.0.0.1:3000.`}</PromptContent>
  </Prompt>
</PromptPanel>
