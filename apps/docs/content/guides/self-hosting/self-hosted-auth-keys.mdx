...
Envoy is the default API gateway and performs the same opaque-key translation - see the [Envoy API Gateway guide](/docs/guides/self-hosting/self-hosted-envoy#authentication). The section below applies to the optional [Kong gateway override](/docs/guides/self-hosting/self-hosted-envoy#reverting-to-kong).
