<PromptPanel>
  <Prompt value="prompt" expandable>
    ...
    <PromptCopy>{`Help me add Supabase to my SolidJS project. Create a Supabase project at
...
5. Run \`npm run dev\` and open http://localhost:3000.`}</PromptCopy>
    <PromptContent>{`Help me add Supabase to my SolidJS project. Create a Supabase project at
database.new and run the instruments table SQL. Then:
1. Run \`npx degit solidjs/templates/js my-app\` to scaffold the app.
2. Run \`npm install @supabase/supabase-js\`.
3. Create \`.env.local\` and set \`VITE_SUPABASE_URL\` and
...
4. Update \`src/App.jsx\` to create a Supabase client and fetch and display the
   instruments table using \`createResource\`.
5. Run \`npm run dev\` and open http://localhost:3000.`}</PromptContent>
  </Prompt>
</PromptPanel>
