...
- `confirmAction`: Preset action verb (`delete`, `purge`, `disable`, `re-enable`, `proceed`) that sets the required confirmation string and label copy. Use `custom` (default when using `confirmString`) for identity verification or other bespoke strings.
- `confirmSubject`: Noun phrase for preset label copy, e.g. `"organization"` → “Type DELETE to delete this organization.”
