<PromptPanel>
  <Prompt value="prompt" expandable>
    ...
    <PromptCopy>{`Help me add Supabase to my Refine project. Create a Supabase project at
...
4. Update \`src/App.tsx\` to add routes for the instruments list, create, edit,
   and show pages.
5. Run \`npm run dev\` and open http://localhost:5173/instruments.`}</PromptCopy>
    <PromptContent>{`Help me add Supabase to my Refine project. Create a Supabase project at
database.new and run the instruments table SQL. Then:
1. Run \`npm create refine-app@latest -- --preset refine-supabase my-app\` to
   scaffold the app with Supabase pre-configured.
2. Update \`src/utility/supabaseClient.ts\` with your Supabase URL and publishable
...
4. Update \`src/App.tsx\` to add routes for the instruments list, create, edit,
   and show pages.
5. Run \`npm run dev\` and open http://localhost:5173/instruments.`}</PromptContent>
  </Prompt>
</PromptPanel>
