...
High-volume log settings (labeled) can degrade database performance and increase logging costs. Configure them with caution for debugging or mandatory compliance.
...
Can produce high levels of log activity, especially for servers with frequent connects/disconnects.
...
Setting `log_min_messages` to severity levels above `warning` (e.g., `info`, `debug1`) can generate excessive logs that degrade database performance. These permissive levels are often intended for temporary debugging or internal development visibility, not general production observability.
...
To avoid excessive logging and mild performance penalties, it is not advised to configure `auto_explain.log_min_duration` below `1s` except briefly for debugging.

</Admonition>

[auto_explain](https://www.postgresql.org/docs/current/auto-explain.html) is a Postgres module that is installed on all Supabase projects. It logs the statements and [explain plans](postgresql.org/docs/current/sql-explain.html) of queries that took more than setting's limit.
...
To avoid excessive logging and mild performance penalties, it is not advised to configure `log_min_duration_statement` below `1s` except for brief periods to help with optimization and debugging.
...
To avoid excessive logging and mild performance penalties, it is not advised to configure `log_duration` except for brief periods to help with optimization and debugging.
...
To avoid excessive logging and performance penalties, it is not advised to configure `log_statement` with the configs the `mod` and `all`. Consider using `pg_audit` over `log_statement` for more granular control over query logging.
...
Determines if a log event should record the query that induced it as a subfield if possible. By default all `ERROR` level and below logs record the query that induced them. To view the severity levels, reference [log_min_messages](#log_min_messages).
