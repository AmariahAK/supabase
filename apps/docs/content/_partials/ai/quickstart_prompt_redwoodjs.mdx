<PromptPanel>
  <Prompt value="prompt" expandable>
    ...
    <PromptCopy>{`Help me add Supabase to my RedwoodJS project. Create a Supabase project at
...
7. Run \`yarn rw dev\` and open http://localhost:8910/instruments.`}</PromptCopy>
    <PromptContent>{`Help me add Supabase to my RedwoodJS project. Create a Supabase project at
database.new and copy the Transaction and Session pooler connection strings.
Then:
1. Run \`yarn create redwood-app my-app --ts\` to scaffold the app.
2. Set \`DATABASE_URL\` (Transaction pooler with \`?pgbouncer=true\`) and
...
5. Update \`scripts/seed.ts\` with instrument data and run \`yarn rw prisma db
   seed\`.
6. Run \`yarn rw g scaffold instrument\` to scaffold the CRUD UI.
7. Run \`yarn rw dev\` and open http://localhost:8910/instruments.`}</PromptContent>
  </Prompt>
</PromptPanel>
