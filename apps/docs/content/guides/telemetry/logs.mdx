...
ClickHouse became the default engine in June 2026. Projects created before then queried logs on BigQuery, whose `cross join unnest(metadata)` syntax is deprecated. Rewrite those queries in the ClickHouse syntax shown here.
...
If you use the [Supabase MCP server](/docs/guides/getting-started/mcp), its `get_logs` tool returns a service's logs from the last 24 hours without writing SQL.
