...
Envoy is registered as the `api-gw` service and also exposes `kong` and `envoy` as network aliases. Either hostname resolves to whichever gateway is active, so internal configs that reference `api-gw:8000` or `kong:8000` (for example, in Edge Functions or Studio) keep working without changes.

{/* supa-mdx-lint-disable-next-line Rule001HeadingCase */}

### Reverting to Kong

Kong is available as an optional override. If your stack is already running, bring it down first with `sh run.sh stop`, then enable the Kong override and start the stack:
...
This replaces Envoy with Kong on the same port (default `8000`) and re-adds Kong's built-in HTTPS listener on `8443`. The `api-gw` service name and the `kong` alias are unchanged, so dependent services keep working.
...
| `/rest/v1/*`                              | rest      | `/`                      | API key        | PostgREST data endpoints                                               |
...
| `/pg/`                                    | meta      | `/`                      | API key        | postgres-meta / Studio database operations (requires secret key)       |
...
- `/pg/` - only `service_role` keys (`sb_secret_*` or legacy `SERVICE_ROLE_KEY`) are allowed
- `/rest/v1/` (exact path, OpenAPI schema root) - only `service_role` keys are allowed
- `/rest/v1/<table>` and other Data API paths remain accessible to all valid keys
...
- **Adding a new environment variable.** Placeholders in the template use the `${VAR_NAME}` form. If you add a placeholder, update both the `api-gw` service in `docker-compose.yml` (to pass the variable into the container) and `docker-entrypoint.sh` (to substitute it with `sed`).
